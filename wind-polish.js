...
function drawKites(w,h,t){
  ...
  for(let i=0;i<count;i++){
    const k=kites[i],anchorX=w*(k.x-.07),anchorY=h*.84,baseX=w*k.x,baseY=h*k.y-vw*.78,bob=Math.sin(t*(.00155+Math.min(.0014,vw*.000018))+k.p)*(6+vw*.12),swayX=d.x*(16+vw*.95)+Math.sin(t*.0011+k.p)*6*gustRatio,x=baseX+swayX,y=baseY+bob,size=23+Math.min(7,vw*.16),rot=heading+Math.sin(t*.0019+k.p)*(.05+Math.min(.06,vw*.001));
    ctx.strokeStyle=isDay?'rgba(255,255,255,.72)':'rgba(220,225,235,.4)';ctx.lineWidth=1.15;ctx.beginPath();ctx.moveTo(anchorX,anchorY);ctx.quadraticCurveTo((anchorX+x)*.52-d.x*24,anchorY-95-vw*1.1,x,y+size*.72);ctx.stroke();
